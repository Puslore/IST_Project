...
```mermaid
flowchart LR
    P[Пассажиры<br/>(просмотр, поиск рейсов)] -->|просмотр| T[Терминал ИС<br/>(отображение расписания, статусов)]
    O[Операторы/службы аэропорта<br/>(ввод данных, обновление)] -->|обновление| T
    T <-->|информация| P
    T <-->|информация| O
```
